...
class GnP_agent(MCTS_agent):
    def __init__(self, *args, **kwargs):
        agent_args = kwargs.pop("agent_args")
        self.grab_thres = agent_args["thres_grab"]
        self.put_thres = agent_args["thres_put"]
        self.start_at_put = agent_args["start_at_put"]
    ...
    def update_curr_goal(self, particles, helper_grab, should_replan):
        ...
        if should_replan["grab"]:
            grab, prob = particles.best_grab()
            if prob < self.grab_thres:
                self.curr_goal = None
            else:
        ...
        # ^ plan for put {on/inside_???_???: ???}
            ...
            put, prob = particles.best_put()
            if prob < self.put_thres:
                self.curr_goal = None
            else:
